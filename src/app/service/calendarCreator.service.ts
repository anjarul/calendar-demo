...
export class CalendarCreator {
  ...
  public getMonth(monthIndex: number, year: number): Day[] {
    let days = [];
    let appointmentList: Map<string, any> = new Map(JSON.parse(localStorage.getItem('appointments')));

    let firstday = this.createDay(1, monthIndex, year, appointmentList.get(this.commonService.generateUniqueKey(1, monthIndex, year)));
    ...
    for (let i = 2; i < countDaysInMonth + 1; i++) {
      days.push(this.createDay(i, monthIndex, year, appointmentList.get(this.commonService.generateUniqueKey(i, monthIndex, year))));
    }
  ...
  private createDay(dayNumber: number, monthIndex: number, year: number, exAppointment) {
    let day = new Day();
    ...
    day.appointmentList = exAppointment;
